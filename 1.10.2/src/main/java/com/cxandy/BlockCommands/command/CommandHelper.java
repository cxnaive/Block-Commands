    ...
    public boolean PostExcute(EntityPlayer player){
        ...
        if(!RequiredItemName.equals(NoSpecialItem)){
            ...
            try {
                ...
                player.addChatComponentMessage(new TextComponentTranslation("Invaild Requried Item:\" %1$s \".",RequiredItemName));
                return true;
            }
            int itemcnt = 0;
            for(ItemStack stack:player.inventory.mainInventory){
                if(stack != null && stack.getItem().getClass() == required.getClass()){
                    itemcnt += stack.stackSize;
                }
            }
            if(itemcnt == 0 || (RequiredItemAmount != Notdefined && itemcnt < RequiredItemAmount)){
                player.addChatComponentMessage(new TextComponentTranslation("Requried Item:\" %1$s \" needs at least %2$s.",RequiredItemName,RequiredItemAmount==Notdefined?"1":String.valueOf(RequiredItemAmount)));
                ...
                for(ItemStack stack:player.inventory.mainInventory){
                    if(stack != null && stack.getItem().getClass() == required.getClass()){
                        int nowreduce = Math.min(itemcnt,stack.stackSize);
                        stack.stackSize -= nowreduce;
                        itemcnt -= nowreduce;
                        if(itemcnt == 0) break;
                    }
                }
        ...
        if(!messagestr.equals(NullMessage)){
            player.addChatComponentMessage(new TextComponentString(messagestr));
        }
