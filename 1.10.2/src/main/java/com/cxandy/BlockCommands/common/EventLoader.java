...
public class EventLoader {
    ...
    public void onPlayerRightClickBlock(PlayerInteractEvent.RightClickBlock event){
        ...
        if(player.hasCapability(CapabilityLoader.commandstorer,null)){
            ...
            blockdata data = blockdata.get(player.worldObj);
            if(cmdstr.isAddingMode){
                ...
                if(nowstr != null){
                    ...
                    if(nowc.MergeCommandFromAddingMode(cmdstr)){
                        ...
                        player.addChatComponentMessage(new TextComponentString("Successfully updated the last command on this block."));
                        player.addChatComponentMessage(new TextComponentString("Now the Command is:"));
                        player.addChatComponentMessage(new TextComponentString(nowc.printString()));
                    }
                    else{
                        player.addChatComponentMessage(new TextComponentString("Invalid Merge Request."));
                    }
                }
                else{
                    player.addChatComponentMessage(new TextComponentTranslation("This block dosen't have an command yet."));
                }
            }
            else if(cmdstr.isDeleteMode){
                if(data.getcommand(longpos) != null){
                    data.remove(longpos);
                    player.addChatComponentMessage(new TextComponentTranslation("Successfully removed command on this block."));
                }
                else{
                    player.addChatComponentMessage(new TextComponentTranslation("This block dosen't have an command yet."));
                }
                ...
                if(datastr == null) player.addChatComponentMessage(new TextComponentTranslation("This block dosen't have an command yet."));
                else{
                    player.addChatComponentMessage(new TextComponentTranslation("Commands on this block are:"));
                    String[] cmds = datastr.split("&&");
                    for(int i = 0;i<cmds.length;++i){
                        player.addChatComponentMessage(new TextComponentString(CommandHelper.loadString(cmds[i]).printString()));
                    }
                }
            }
            else{
                ...
                if(nowcmd == null) nowcmd = cmdstr.toString();
                    ...
                    player.addChatComponentMessage(new TextComponentTranslation("This Block Already has this command."));
                    return;
                }
                else nowcmd = nowcmd + "&&" + cmdstr;
                data.update(longpos,nowcmd);
                player.addChatComponentMessage(new TextComponentTranslation("Successfully updated command on this block."));
            }
        }
    }
    @SubscribeEvent
