    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if(args.length == 4){
            ...
            try{
                ...
                sender.sendMessage(new TextComponentString("PosX,PosY,PosZ and Distance must be Intergers."));
                return;
            }
        }
            ...
            if(sender instanceof EntityPlayer){
                try {
                    ...
                    ((EntityPlayer) sender).sendMessage(new TextComponentString("Distance must be a Interger."));
                    return;
                }
            }
            else{
                sender.sendMessage(new TextComponentString("/commandfind [Distance] can only be excuted by player."));
                return;
            }
        }
        else{
        ...
        for(long o : hasher){
            ...
            if(Math.abs(npos.getX()-qpos.getX()) <= distance && Math.abs(npos.getY()-qpos.getY()) <= distance && Math.abs(npos.getZ()-qpos.getZ()) <= distance){
                sender.sendMessage(new TextComponentString("X:"+npos.getX()+" Y:"+npos.getY()+" Z:"+npos.getZ()+"."));
                ++count;
            }
        }
        sender.sendMessage(new TextComponentString("Showd "+count+" blocks with commands in total."));
    }
}